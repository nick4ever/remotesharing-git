var DataTable = function () {
        ...
        if (all) {
            if (checkbox.checked) {
                if (dataTable.totalItems > 200) {
                    Dialog.error("Selecting more than 200 items is not supported.");
    ...
    SelectorColumn.prototype.getTitleContentHtml = function () {
        return this.exclusive ? "" : "<input type=\"checkbox\" class=\"DataTableCheck All\" />";
    };
    ...
    ActionColumn.prototype.getCellContentHtml = function (data, row, col) {
        ...
        for (var i = 0; i < this.actions.length; i ++) {
            ...
            html += "<hbox><hbox style=\"width: 100%\"></hbox><span action-id=\"" + action.id + "\" class=\"" + action.type + " Action"  + action.id +  " " + (action.isApplicable && action.isApplicable(data) ? "" : "disabled")  + "\" title=\"" + Dom.htmlEncode(action.title) + "\"><icon class=\"" + action.type +"\"></icon></span></hbox>"
        }
        return html;
    };
    ...
    DataTable.prototype.addSelectionInfoPane = function() {
        ...
        selectionItemDataTable.column(new DataTable.ActionColumn(actions).width("3em"));
